...
class RobotManager:
    def __init__(self, b_width, b_height, obs_size=100):
        ...
        self.frame_top = None
        self.corners = None
        self.pickup_p = (110, -168) # location to pickup the checker
    ...
    def warp_board(self, tags):
        ...
        rect = np.array(self.corners).reshape(4, 2).astype('float32')
        maxWidth = 300
        maxHeight = 400
        dst = np.array([
            [0, 0],
        ...
        warped = cv2.warpPerspective(self.frame_top, M, (maxWidth, maxHeight))
    ...
    def flip(self):
        # self.win()
        self.parse_board()
        pos1 = self.query_position((self.corners[0] + self.corners[1]) / 2)
        pos2 = self.query_position((self.corners[2] + self.corners[3]) / 2)
        if pos1[0] < pos2[0]:
            pos = pos1
        else:
            pos = pos2
